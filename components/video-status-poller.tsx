...
import { getAssetsStatus } from "@/app/actions";
import { Loader2 } from "lucide-react";
import { useRouter } from "next/navigation";
import { useEffect } from "react";

const VideoStatusPoller = ({
  id,
  isVideoReady,
}: {
  id: string;
  ...
  useEffect(() => {
    const checkStatus = async () => {
      const { transcript, transcriptStatus } = await getAssetsStatus(id);
...
export default VideoStatusPoller;
